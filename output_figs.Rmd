...
# plot
gt=grid.arrange(arrangeGrob(p1, p2, nrow=1),arrangeGrob(p3, p4, nrow=1), nrow=2)
p=as_ggplot(gt) +
...
plot.df$flag.var.label = factor(plot.df$flag.var.label, levels = c("0 copies", "1-10 copies", ">10 copies"))
...
load(file=here("./out/figS16a.Rda"))
p1=ggplot(data = plot.df,
         aes(x = hs, y = as.factor(sample), fill=flag.var.label)) +
...
load(file=here("./out/figS16b.Rda"))
...
load(file=here("./out/figS16c.Rda"))
p3=ggplot(data = plot.df,
         aes(x = hs, y = as.factor(sample), fill=flag.var.label)) +
  scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),labels = trans_format("log10", math_format(10^.x))) +
  scale_fill_manual(values=c("black","#882255","#44AA99","#6699CC")) +
...
load(file=here("./out/figS16d.Rda"))
...
pdf(here("./out/Figure 4-figure supplement 5.pdf"),  height=6, width=7.5, useDingbats=FALSE)
print(p)
dev.off()
