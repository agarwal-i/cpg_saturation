...
set.seed(as.numeric(array))
df = setDT(as.data.frame(rgamma(nsim, shape = 0.23, scale = 425/0.23)/10000))
colnames(df) = "s"
df$h = h
...
head(df)
write.table(df, file=paste0("abc2.params.",args[4],".txt"), quote=F, row.names=F, col.names=F, sep=" ")
